...
describe('Tests for cleanupBlocks option', () => {
    ...
    it('Should trim extra spaces in three blocks', () => {
        let result = jlto.optimizeString('<div>{%  extends "base.html"  %}</div><div>{%  extends "base.html"  %}</div><div>{%  extends "base.html"  %}</div>', {
            cleanupExpressions: true
        });

        assert.equal('<div>{%extends "base.html"%}</div><div>{%extends "base.html"%}</div><div>{%extends "base.html"%}</div>', result);
    });
